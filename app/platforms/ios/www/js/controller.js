...
screen_registro.show = function(){screen_registro.removeClass('downed');};
        ...
        complete: function(a,b,c){
        	if(user_data.msj==undefined){
                ...
                screen_timeline.nfeed.fadeOut('fast');
           		...
           		navigator.notification.alert('Usuario y/o contraseña incorrectos', null, 'Voz de Dios','ok');
                user_data = {};
            }
            $("#button_facebook_connect").removeClass("loading");
            console.log(user_data.data.premium);
        }
    });
}
...
function send_desuscription(){
	if($("#send_desuscription").hasClass('loading')) return false;
    
	navigator.notification.confirm(
        '¿Estás seguro que deseas desuscribirte?',
        function onConfirm(buttonIndex) {
        	if(buttonIndex==1){
        		$("#send_desuscription").addClass("loading");
				$.ajax({
			    	url: urlws,
			        dataType: 'text',
			        type: 'post',
...
function invite(){
	var url = 'http://lvd.pizotesoft.com/';
